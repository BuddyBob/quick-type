...
const Home = () =>  {
  ...
  const [prevUserInput, setPrevUserInput] = useState(initialState.previousUserInput);
  console.log(state)
  function newThing(){
  ...
  function onUserInputChange(e){
    ...
    if ((v.length>state.previousUserInput.length) && (v[v.length-1]!==state.text[v.length-1])){
      state.setState({errors:state.errors + 1})
    }
    prevUserInput.setState(setPrevUserInput)
