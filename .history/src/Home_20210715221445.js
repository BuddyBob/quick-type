...
const Home = () =>  {
  ...
  // set hooks
  const [loggedIn,setLoggedIn] = useState(localStorage.getItem("currentUserId") ? true : false)
  //only define a user id if user is logged in
  const [userId, setUserId] = useState(localStorage.getItem("currentUserId"))
  const [englishType,setEnglishType] = useState(loggedIn ? data.englishType : localStorage.getItem("englishType"))
  ...
  const [text,setText] = useState(loggedIn ? GetText(data.wordCount,data.englishType) : GetText(localStorage.getItem('wordCount'),localStorage.getItem('englishType')))
  ...
  //get new text
  function newText(){
    setText(loggedIn ? GetText(data.wordCount,data.englishType) : GetText(localStorage.getItem('wordCount'),localStorage.getItem('englishType')))
  }
  //logout
      ...
      <div className="container mt-5 mb-5" style={{width:'100%'}}>
        <div className="row">
          <div className="col-md-6 offset-md-3">
            ...
            <div style={{focus:"2"}}>
            <Preview wordCount={loggedIn ? data.wordCount : localStorage.getItem('wordCount')} text={text} userInput={userInput}/>
            </div>
            <form className="row g-4">
              ...
              <div className="col-auto">
                <Wpm EnglishType={loggedIn ? data.englishType : localStorage.getItem('englishType')} errors={errors} userInput={userInput} sec={sec} symbols={symbols}/>
              </div>
