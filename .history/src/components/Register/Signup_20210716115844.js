        ...
        async function handleSubmit(e) {
            ...
            if (passwordRef.current.value === passwordConfirmRef.current.value){
                try {
                    ...
                    //log user in setting there currentUser id
                    db.collection("users").doc(currentUser.uid).set({
                        email: emailRef.current.value,
                        password: passwordRef.current.value,
                        wordCount:localStorage.getItem("wordCount") ? localStorage.getItem("wordCount") : "10",
                        englishType:localStorage.getItem("englishType") ? localStorage.getItem("englishType") : "english",
                    })
                    .then((docRef) => {
