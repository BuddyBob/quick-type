...
const SettingsButtons = ({runFunction, types, change, additionalText}) => {
    ...
    // }
    function getIndex(){
        db.collection('users').doc(currentUser.uid).get().then((docRef) => { await setWordCount(docRef.wordCount) })
        return types.indexOf()
    }
    const [active, setActive] = useState(types[getIndex()]);
    return (
        <div>
            {types.map((type) => (
