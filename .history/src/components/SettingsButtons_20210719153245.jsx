...
const SettingsButtons = ({dbData, runFunction, types, change, additionalText}) => {
    ...
    async function changed(e){
        ...
        if (change === "wordCount"){await db.collection("users").doc(userId).update({wordCount:val})}
        else if (change === "englishType"){await db.collection("users").doc(userId).update({englishType:val})}
    ...
    return (
        ...
        { change == "englishType" &&
            <div>
                <input onChange={changed}   placeholder={dbData ? dbData.englishType : null} type="text" style={{width:"250px",marginBottom:"20px"}}/>
            </div>
        }
        </div>
    )
