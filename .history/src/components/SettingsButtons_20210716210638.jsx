...
const SettingsButtons = ({runFunction, types, localStorageName, additionalText}) => {
    ...
    function updateDb(){
        db.collection('users').doc(currentUser.uid).update({wordCount:wrdCount})
        index = setActive(types.indexOf(wrdCount))
    }

    function previousData(kind,data){
        if (kind === 'changeWordCount'){
            //get previous wordCount
            db.collection('users').doc(currentUser.uid).get().then((docRef) => { setWordCount(data) })
            //update data with this
            updateDb()
        }
    ...
    return (
        <div>
            {types.map((type) => (
            <ButtonToggle active={active === type} onClick={() => previousData(runFunction,active.split(' ')[0])}>
                {type+additionalText}
            </ButtonToggle>
            ))}
            <p>{}</p>
