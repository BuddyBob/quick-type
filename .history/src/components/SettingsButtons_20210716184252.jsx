...
const SettingsButtons = ({runFunction, types, localStorageName, additionalText}) => {
    ...
    if (currentUser === null){
        const [englishType,setEnglishType] = useState(currentUser === null ? localStorage.getItem("englishType") : returnUserData(userId).then(result => {result.englishType}))
        const [wordCount,setWordCount] = useState(currentUser === null ? localStorage.getItem("wordCount") : returnUserData(userId).then(result => {result.englishType}))
    }
